...
class TestController < ApplicationController
  def index
    ...
    # context = ExecJS.compile_async(Rails.root.join('js', 'domcore.js').read)


    # ExecJS.exec('var jsdom = require("jsdom");')
...
#     JS
    ...
    # render text: "x: <pre>#{ context.call('run', '<p>Hello World</p>') }</pre>"

    # ENV['NODE_PATH'] = Rails.root.join('js').to_s
#     context = ExecJS.compile <<-JAVASCRIPT
# //      var path = require("path");
#       var jsdom = require('jsdom');
# //      var domcore = require('domcore');
# //      var weld = require('weld');
#       var f = function() { return jsdom; };
#     JAVASCRIPT
    ...
    render text: "x: #{ context.call 'run' }"
  end
end